...
export default function ProductOrdersPage() {
  ...
  async function loadItems() {
    ...
    try {
      ...
      const productOrders = (Array.isArray(res.data) ? res.data : [])
  ...
  async function remove(id) {
    if (!confirm("Delete this product order?")) return;
      ...
      <div className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-pink-500/20 via-white/[0.05] to-violet-500/10 p-6 shadow-2xl">
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          <div>
            ...
            <h1 className="text-4xl font-black">Product Orders</h1>

            <p className="mt-3 text-sm leading-7 text-white/55">
              Manage customer product order requests, quantity, total amount,
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_220px_160px]">
        <label className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3">
          ...
          <input
            ...
            placeholder="Search name, phone, product, address, message..."
          ...
          <table className="w-full min-w-[1200px]">
            <thead className="bg-white/[0.07]">
              <tr className="text-left text-xs font-black uppercase tracking-widest text-white/45">
                ...
                <th className="px-5 py-4">Address / Message</th>
                  ...
                  >
                    ...
                    <td className="px-5 py-4 align-top">
                      <p className="max-w-[220px] font-bold text-white">
                        {item.product_title || "Product order"}
                      </p>
                    ...
                    <td className="px-5 py-4 align-top">
                      <p className="font-black text-pink-200">
                        LKR {Number(item.total_amount || 0).toLocaleString()}
                      </p>
                    </td>

                    <td className="px-5 py-4 align-top">
                      <p className="max-w-[300px] line-clamp-2 text-sm leading-6 text-white/55">
                        {item.address || "No address"}
                      </p>

                      {item.message && (
                        <p className="mt-2 max-w-[300px] line-clamp-2 text-xs leading-5 text-white/40">
                          {item.message}
                        </p>
                      )}
                    </td>
