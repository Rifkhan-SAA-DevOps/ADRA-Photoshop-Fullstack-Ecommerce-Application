...
function getStatusStyle(status) {
  const styles = {
    ...
    completed: "bg-green-700/20 text-green-100 border-green-300/20",
...
function getBookingType(item) {
  if (item.type === "product_order" || item.product_id || item.product_title) {
      ...
      <div className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-violet-500/20 via-white/[0.05] to-pink-500/10 p-6 shadow-2xl">
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          <div>
            ...
            <h1 className="text-4xl font-black">Booking Requests</h1>

            <p className="mt-3 text-sm leading-7 text-white/55">
              View and manage event bookings and product order requests.
            </p>
          </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_220px_160px]">
        <label className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3">
          ...
          <input
            ...
            placeholder="Search name, phone, product, event, message..."
          ...
          <table className="w-full min-w-[1200px]">
            <thead className="bg-white/[0.07]">
              <tr className="text-left text-xs font-black uppercase tracking-widest text-white/45">
                ...
                <th className="px-5 py-4">Item / Event</th>
                <th className="px-5 py-4">Qty / Total</th>
                  ...
                  return (
                    ...
                    >
                      <td className="px-5 py-4 align-top">
                        <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-black text-white/70">
                          {isProductOrder ? (
                            <Package size={13} />
                          ) : (
                      ...
                      <td className="px-5 py-4 align-top">
                        ...
                        {item.service_needed && (
                          <p className="mt-2 max-w-[220px] text-xs text-white/45">
                            {item.service_needed}
                          </p>
                        )}
                      </td>
                      ...
                      <td className="px-5 py-4 align-top">
                        {isProductOrder ? (
                          ...
                          <p className="text-sm text-white/40">N/A</p>
                        )}
                      </td>

                      <td className="px-5 py-4 align-top">
                        <p className="max-w-[260px] line-clamp-3 text-sm leading-6 text-white/55">
                          {item.message || item.address || "No message"}
                        </p>
                      </td>
